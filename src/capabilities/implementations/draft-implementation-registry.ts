...
export class SyntheticDraftImplementationRegistry implements SyntheticImplementationRegistryPort {
  ...
  constructor(entries: readonly CapabilityEntry[]) {
    for (const entry of entries) {
      const adapter = syntheticDraftAdapters.get(entry.capabilityId);
      if (adapter === undefined) {
        continue;
      }
      this.#registrations.set(entry.capabilityId, {
        capabilityId: entry.capabilityId,
        binding: syntheticBindingFor(entry, "approval://synthetic/s4-local-test-only"),
        adapter
      });
    }
    if (this.#registrations.size !== syntheticDraftAdapters.size) {
      throw new Error("synthetic draft implementation registry is incomplete");
    }
  }
