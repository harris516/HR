...
- 由受控测试 Feishu Bot 与测试发送者发问，读取一条 synthetic `OnboardingCase`，生成 Day-1 Ready 建议卡草稿；模型只负责解释工具返回的结构化结果。同一个 `aibang-hr-onboarding-agent` 可承接多个独立 Bot。
...
`pnpm typecheck`、`pnpm build:mvp`、`pnpm test` 通过；最终测试数量见 `STEP1_IMPLEMENTATION_REPORT.md`。`seed:mvp` 已在新的本地临时数据库生成 Team-scoped Case，Case Version 为 4。变更后的多 Bot 主体插件配置仍须在服务器版 OpenClaw 上重新校验后才能启用。
...
5. 插件配置必须明确设置 `trustedPrincipals`、`databasePath`（上述仓库外绝对路径）、`repositoryRoot`（上述 HR 仓库绝对路径）。每个可信主体包含运行时 Feishu `accountId` 与 `open_id`、Tenant、Team DataSpace、Actor、active Team、membership、角色和 scope grants；`(accountId, open_id)` 必须唯一。真实 `accountId`、`open_id` 和 Secret 不写入 Git。安装本地链接插件前，审阅其 `contracts.tools`，仅同意 `aibang_hr_onboarding_test_ready_card`。
